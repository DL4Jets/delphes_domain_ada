...
def run_model(outdir, training_method=1):
	...
	def modelIverseGrad(Inputs,rev_grad):
		#build the models
		feat, outshape=common_features(Inputs)
		btag,_=btag_discriminator(outshape,dropoutrate=0.03)
		datamc,_=datamc_discriminator(outshape,rev_grad=rev_grad,dropoutrate=0.03)
	...
	if(training_method == 'MC_training'):
		...
		feat, outshape=common_features(Inputs)
		btag,_=btag_discriminator(outshape,dropoutrate=0.03)
		datamc,_=datamc_discriminator(outshape,rev_grad=args.lmb,dropoutrate=0.03)
